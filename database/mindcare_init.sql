...
SELECT
    ...
    SUM(CASE WHEN i.intervention_id IS NOT NULL THEN 1 ELSE 0 END) AS intervention_count
FROM assessments a
...
SELECT
    ...
    ROUND(AVG(a.total_score), 2) AS avg_score,
    SUM(CASE WHEN a.level IN ('moderate','severe') THEN 1 ELSE 0 END) AS risk_count,
    ROUND(SUM(CASE WHEN a.level IN ('moderate','severe') THEN 1 ELSE 0 END) * 100.0
          / NULLIF(COUNT(a.assess_id), 0), 2) AS risk_rate_pct,
    MAX(a.assessed_at) AS last_used
FROM scales s
...
SELECT
    ...
    c.specialty,
    c.available,
    COUNT(i.intervention_id) AS total_tasks,
    ...
    MAX(i.created_at) AS last_task_time
FROM counselors c
...
SELECT
    ...
    a.total_score AS last_score,
    a.level AS last_level,
    a.assessed_at AS last_assess_time,
    COUNT(i.intervention_id) AS intervention_count,
    SUM(CASE WHEN i.status IN ('pending','in_progress') THEN 1 ELSE 0 END) AS active_interventions
FROM users u
JOIN risk_levels rl ON u.user_id = rl.user_id AND rl.risk_level IN ('warning','crisis')
...
WHERE u.department != '心理咨询中心'
GROUP BY u.user_id, u.student_id, u.name, u.department, u.grade, u.phone,
         rl.risk_level, rl.risk_score, s.name, a.total_score, a.level, a.assessed_at
...
SELECT
    DATE_FORMAT(a.assessed_at, '%Y-%m') AS month,
